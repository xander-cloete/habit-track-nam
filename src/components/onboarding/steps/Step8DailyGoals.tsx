...
export default function Step8DailyGoals() {
  const { data, nextStep, prevStep, updateData } = useOnboardingStore();

  const [goals, setGoals] = useState<string[]>(() => {
    const prefilled = (data.dailyGoals ?? []).filter((g) => g.trim() !== '');
  ...
  const handleContinue = () => {
    ...
    updateData({ dailyGoals: filtered });
    nextStep();
  };
  ...
  return (
    ...
    >
      ...
      <p
        ...
        style={{ color: '#8B7355' }}
      >
        What are the 1-3 things you must do every single day, no matter what?
      </p>
        ...
        {goals.map((goal, index) => (
          <div key={index} className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder={
                index === 0
                  ? 'e.g. Meditate 10 min, Drink 2L water...'
