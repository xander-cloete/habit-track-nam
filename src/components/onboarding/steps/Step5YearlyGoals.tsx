...
export default function Step5YearlyGoals() {
  const { data, nextStep, prevStep, updateData } = useOnboardingStore();

  const [goals, setGoals] = useState<string[]>(() => {
    const prefilled = (data.yearlyGoals ?? []).filter((g) => g.trim() !== '');
  ...
  const handleContinue = () => {
    ...
    updateData({ yearlyGoals: filtered });
    nextStep();
  };
  ...
  return (
    ...
    >
      ...
      <p
        ...
        style={{ color: '#8B7355' }}
      >
        Think big. What would make this year incredible?
      </p>
        ...
        {goals.map((goal, index) => (
          <div key={index} className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder={
                index === 0
                  ? 'e.g. Run a marathon, Launch my business...'
                  : `Goal ${index + 1}...`
              }
